...
`:app`, `:core:domain`, `:core:data`, or another feature. All feature events pass through the feature's single
`send(event)` surface, and all shared-state changes pass through verb-named `AppStore` actions.
